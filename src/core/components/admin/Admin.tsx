...
import { ArrowLeft, Filter, Search, X, LoaderCircle, AlertTriangle, Trash2, Swords } from 'lucide-react';
...
const AdministrarGrupoJuego = () => {
  ...
  const [infoJuego, setInfoJuego] = useState<{juego: string, grupo: string, total: number} | null>(null);
  ...
  // Estado para almacenar el ID del ganador por partida
  const [winners, setWinners] = useState<{ [key: number]: string | null }>({});
  ...
  const fetchPartidas = useCallback(async (juegoId: string, grupoId: string) => {
    ...
    setError(null);
    setPartidas([]); 
    setWinners({}); // Limpiar ganadores al buscar de nuevo
    ...
    try {
      ...
      if (!data.partidas || data.partidas.length === 0) {
        setInfoJuego({juego: data.details.juego, grupo: data.details.grupo, total: 0});
        setPartidas([]);
      } else {
        const partidasTransformadas: Partida[] = data.partidas.map(p => ({
          id: p.id_partida,
          jugadores: [
        ...
        setPartidas(partidasTransformadas);
        setInfoJuego({juego: data.details.juego, grupo: data.details.grupo, total: data.details.total_partidas});
      }
    } catch (err) {
  ...
  // Función para cambiar el nivel de un jugador
  const cambiarNivel = async (partidaId: number, jugadorId: string, incremento: number) => {
    ...
    try {
        ...
        setPartidas(current => current.map(p => p.id === partidaId ? { ...p, jugadores: p.jugadores.map(j => j.id === jugadorId ? { ...j, nivel: j.nivel + incremento } : j) } : p));
        
        // Lógica de ganador
        if (incremento > 0) {
            setWinners(prev => ({ ...prev, [partidaId]: jugadorId }));
        } else {
            if (winners[partidaId] === jugadorId) {
                setWinners(prev => ({ ...prev, [partidaId]: null }));
            }
        }
  ...
  // Función para eliminar las partidas del grupo actual
  const handleDeletePartidas = async () => {
    ...
    try {
        ...
        setPartidas([]);
        setInfoJuego(prev => prev ? { ...prev, total: 0 } : null);
    } catch (err) {
      ...
      <main style={{ display: 'grid', gridTemplateColumns: '1fr 360px', gap: '12px', flex: 1, minHeight: 0 }}>
        ...
        <div style={{ background: 'white', borderRadius: '8px', padding: '16px', display: 'flex', flexDirection: 'column', boxShadow: '0 4px 12px rgba(0, 0, 0, 0.15)', minHeight: 0 }}>
          <div style={{ background: '#10b981', color: 'white', padding: '10px 14px', borderRadius: '6px', marginBottom: '16px', fontSize: '16px', fontWeight: '600', textAlign: 'center', flexShrink: 0 }}>
            {infoJuego ? `Juego ${infoJuego.juego} - Grupo ${infoJuego.grupo} (${infoJuego.total} Partidas)` : 'Partidas en curso'}
          </div>
            ...
            {cargando ? (
              ...
              <div style={{display: 'flex', flexDirection: 'column', gap: '10px'}}>
                {partidasFiltradas.map((partida) => {
                  const winnerId = winners[partida.id];
                  return (
                    <div key={partida.id} style={{ border: `1px solid ${winnerId ? '#d1d5db' : '#10b981'}`, borderRadius: '8px', padding: '12px', background: winnerId ? '#f9fafb' : '#f8fffe', boxShadow: '0 1px 3px rgba(0,0,0,0.05)', transition: 'all 0.3s' }}>
                      <div style={{ color: winnerId ? '#6b7280' : '#dc2626', fontSize: '14px', fontWeight: 'bold', marginBottom: '8px' }}>Partida {partida.id} {winnerId && '(Ganador Seleccionado)'}</div>
        ...
        <aside style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            ...
            <div style={{ background: 'white', borderRadius: '8px', padding: '16px', boxShadow: '0 4px 12px rgba(0, 0, 0, 0.15)' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                ...
                <button onClick={handleCreateRonda} disabled={isCreating || partidas.length > 0} style={{ background: '#22c55e', color: 'white', border: 'none', padding: '12px', borderRadius: '6px', fontSize: '14px', fontWeight: '600', cursor: (isCreating || partidas.length > 0) ? 'not-allowed' : 'pointer', opacity: (isCreating || partidas.length > 0) ? 0.5 : 1, transition: 'background 0.2s', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px' }}>
                  {isCreating ? <><LoaderCircle size={16} className="animate-spin" /> Creando...</> : 'Crear Ronda'}
                </button>
                {partidas.length > 0 && <p style={{fontSize: '12px', color: '#6b7280', textAlign: 'center', margin: '4px 0 0 0'}}>Elimine la ronda actual para crear una nueva.</p>}
              </div>
            </div>
            ...
            <div style={{ background: 'white', borderRadius: '8px', padding: '16px', boxShadow: '0 4px 12px rgba(0, 0, 0, 0.15)', border: '1px solid #f87171' }}>
              ...
              <button onClick={() => setShowDeleteConfirm(true)} disabled={isDeleting || partidas.length === 0} style={{ background: '#ef4444', color: 'white', border: 'none', padding: '12px', borderRadius: '6px', fontSize: '14px', fontWeight: '600', cursor: (isDeleting || partidas.length === 0) ? 'not-allowed' : 'pointer', opacity: (isDeleting || partidas.length === 0) ? 0.5 : 1, transition: 'background 0.2s', width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px' }}>
                {isDeleting ? <><LoaderCircle size={16} className="animate-spin" /> Eliminando...</> : <><Trash2 size={16}/> Eliminar Ronda Actual</>}
              </button>
            </div>
        </aside>
      </main>
      ...
      {showDeleteConfirm && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0, 0, 0, 0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }}>
          <div style={{ background: 'white', padding: '24px', borderRadius: '8px', boxShadow: '0 5px 15px rgba(0,0,0,0.3)', width: '100%', maxWidth: '400px', textAlign: 'center' }}>
            ...
            <p style={{color: '#4b5563'}}>Estás a punto de eliminar todas las partidas del <strong>Juego {idJuego} / Grupo {idGrupo}</strong>. Esta acción no se puede deshacer.</p>
            <div style={{display: 'flex', justifyContent: 'flex-end', gap: '12px', marginTop: '24px'}}>
              <button onClick={() => setShowDeleteConfirm(false)} style={{padding: '10px 20px', borderRadius: '6px', border: '1px solid #d1d5db', background: 'white', cursor: 'pointer', fontWeight: '500'}}>Cancelar</button>
              <button onClick={handleDeletePartidas} style={{padding: '10px 20px', borderRadius: '6px', border: 'none', background: '#dc2626', color: 'white', cursor: 'pointer', fontWeight: '600'}}>Confirmar Eliminación</button>
            </div>
          </div>
        </div>
      )}
